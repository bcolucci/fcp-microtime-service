...
const isString = function (value) { return 'string' === typeof(value); };
const cleanStr = function (str) { return str.replace(/[^a-z0-9\s]/gi, '').trim(); };
const isTimestamp = function (str) { return null !== str.match(/^[0-9]+$/); };

const INVALID_TYPE_ERROR = new Error('Invalid type, string expected.');

const NULL_OUTPUT = { unix: null, natural: null };
const output = function (unix) {
  return {
    unix: unix,
